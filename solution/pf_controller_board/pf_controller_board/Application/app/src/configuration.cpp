...
DmaSpiSlave DmaSPI {
  ...
  PIN_SPI_MISO,  PIN_SPI_SCK,  PIN_SPI_MOSI, SS,  // MISO: MISO (PA12), SCK: SCK (PB11), MOSI: MOSI (PB10), SS: A2(PB08)
...
void SERCOM0_Handler (void) {
  DmaSerial0.errorHandler();
}
void SERCOM2_Handler (void) {
  DmaSerial1.errorHandler();
}  
...
void initAppComponents() {
  ...
  if (pinPeripheral(kPinRxUart0, PIO_SERCOM) < 0) {
    while(1) {}
  }
  pinMode(kPinTxUart0, OUTPUT);
  if (pinPeripheral(kPinTxUart0, PIO_SERCOM) < 0 ) {
    while(1) {}
  }
  ...
  if(pinPeripheral(kPinRxUart1, PIO_SERCOM) < 0 ) {
    while(1) {}
  }
  pinMode(kPinTxUart1, OUTPUT);
  if (pinPeripheral(kPinTxUart1, PIO_SERCOM_ALT) < 0) {
    while(1) {}
  }
  ...
  pinMode(SCK, INPUT);
  if (pinPeripheral(PIN_SPI_SCK, PIO_SERCOM_ALT) < 0) {
    while(1) {}
  }
  pinMode(MISO, OUTPUT);
  if (pinPeripheral(MISO, PIO_SERCOM_ALT) < 0) {
    while(1) {}
  }
  pinMode(MOSI, INPUT);
  ...
  DmaSerial1.begin(kUartBaudrate);
